        ...
        for line in data:
            ...
            for para_id, para in enumerate(coder_dict[0]):
                ...
                for sent_id, sent in enumerate(para):
                    doc["token_labels"].append([args.neg_label for _ in range(len(sent["orig"]))])
                    doc["tokens"].append(sent["orig"])
                    ...
                    for id in coder_ids:
                        ...
                        cor = annotator.parse(" ".join(coder_dict[id][para_id][sent_id]["cor"]))
                        gold_edits = coder_dict[id][para_id][sent_id]["edits"]
                        # Gold edits
                        if args.gold:
                            ...
                            # go through edits and assign positive labels
                            for edit in proc_edits:
                                if edit.o_start == -1 and edit.o_end == -1:
                                    continue
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert BEA2019 Shared Task style JSON to internal JSON format.",
    ...
    parser.add_argument(
        "-neg_label",
        help="Value to use for negative classes",
        default=0)
    parser.add_argument(
        "-pos_label",
        help="Value to use for positive classes",
        default=1)
    parser.add_argument(
        "-sents",
